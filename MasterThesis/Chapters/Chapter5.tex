...
The second section presents benchmarking results obtained while evaluating the OSR inliner, and RJIT OSR main features, i.e., the compensation code to replace invalidated functions, and the \codekey{getFreshIR} function.\\
...
The \codekey{FunctionCall} class provides a static function, called \codekey{getFunctionCalls}, that takes an LLVM IR as input, and extracts the function calls it contains.
For each ic stub call, the function creates an instance of the \codekey{FunctionCall} class. 
A \codekey{FunctionCall} object gives quick access to each element of the function call, i.e., the \codekey{getFunction} call instruction, the arguments to the call, and the additional elements of the ic stub call.
The \codekey{getFunctionCalls} returns a list of \codekey{FunctionCall} instances.
...
All function calls are extracted from the outer function using the \codekey{getFunctionCalls} function.
...
In the default mode, the OSR inliner simply calls the \codekey{getFreshIR} method to obtain the callee's LLVM IR.
...
Function \codekey{g} contains two function calls, one to \codekey{f}, and another to a function \codekey{h} for which we do not have a definition in the current scope.
The IR displayed in Figure \ref{fig:originalouter} corresponds to \codekey{g}.
Lines 3 to 6 correspond to the call to \codekey{f}.
Lines 7 to 10 correspond to the call to the unknown function \codekey{h}.
Figure \ref{fig:originalinner} provides the LLVM IR of \codekey{f}.
We apply the OSR inliner on function \codekey{g}.\\
...
The use of the OSR Handler's \codekey{baseVersions} map, for the Shootout benchmarks, yielded a disappointing result.
...
First, the \codekey{baseVersions} map is intended to avoid re-translating ASTs to LLVM IR, for already compiled functions.
...
As a result, in order to evaluate the efficiency of the \codekey{baseVersions} map, another form of experiment is required.
Section \ref{section:getfreshtest} focuses on the evaluation of the \codekey{getFreshIR} function and the OSR Handler's mechanisms.\\
...
Registering an entry in the \codekey{baseVersions} map increases the cost of a function's first compilation.
...
The reason is that it has to generate the LLVM IR corresponding to the function's AST, as well as cloning the obtained IR and registering the clone in the \codekey{baseVersions} map, while RJIT simply needs to generate the IR.
We evaluate this overhead by breaking down a function's compilation into three steps: 1) the translation, i.e., from GNUR AST to LLVM IR, 2) the registration, i.e, adding an entry in the \codekey{baseVersions} map, 3) the \codekey{jitAll}, i.e., inserting safepoints, patchpoints and generating the native code.
...
If we further compare this result to the overall cost of a complete compilation, i.e., the cost of the translation and \codekey{jitAll}, we obtain a ratio of \textbf{0.004}, with a standard deviation of 0.002. 
...
As a result, we conclude that the overhead introduced by the OSR Handler to populate the \codekey{baseVersions} map is insignificant compared to the overall execution, and acceptable compared to the translation's cost.\\
...
As a result, we need to evaluate the cost of obtaining a non-instrumented and correct IR from the OSR Handler, when the \codekey{baseVersions} map already contains an entry for the function, and compare it to the cost of generating the IR from the AST.
We therefore perform a second experiment, where the OSR Handler has a fully generated \codekey{baseVersions} map.
...
We can further affirm that the \codekey{getFreshIR} performance compensates the overhead introduced while registering an entry in the \codekey{baseVersions} map.\\
...
Since, for the moment, a garbage collection induces a flush of the OSR Handler's \codekey{baseVersions} map, i.e., all entries are deleted, we can say that only live functions, compiled since the last garbage collection, have an entry in the OSR Handler.
