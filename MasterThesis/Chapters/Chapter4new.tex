...
Non-instrumented IRs are registered in the \codekey{base version} map, a map from a SEXP closure to a \codekey{NATIVESXP} function SEXP.
...
The \codekey{getFreshIR} strives to avoid having to re-generate LLVM IR for NATIVESXPs.
...
The \codekey{addRelocation} function enables to register a mapping between a clone SEXP and the cloned LLVM function it contains.
...
Assuming there are more reads then writes, and that the value for \codekey{x} is rarely modified, we can expect the unguarded point to have better performances than a guarded one.\\
