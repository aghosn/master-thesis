...
%R is slow.
    ...
    %Why slow. 
%Goal of the thesis 
    %OSR to enable aggressive opt in the impl
    %Show that it is possible with example
%Reusing a library    
...
Although very flexible and widely used for statistical data analysis, the R programming language and its interpreter suffer from poor execution performances, compared to equivalent programs execution in C or Python\cite{morandat2012evaluating}.
According to \citean{morandat2012evaluating}, on the Shootout benchmarks suite, R is on average 501 times slower than C and 43 times slower than Python.
