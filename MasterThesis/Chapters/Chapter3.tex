...
Providing source level interactive debugging is hard in the presence of optimizations.
...
In \citetitle{holzle1992debugging}~\cite{holzle1992debugging}, the authors came up with a new mechanism that enables to dynamically deoptimize code at specific interrupt points in order to provide source code level debugging while preserving expected behaviors.\\

\citean{holzle1992debugging} present the main challenges encountered to provide debugging behaviors, due to the optimizations performed by the SELF compiler. 
...
Jikes RVM is an extensible framework in which on-stack replacement techniques have been implemented in two steps, i.e., a first implementation~\cite{fink2003design}, and then an extended version of OSR\cite{soman2006efficient}.
%The first
...
Forward arrows represent \textit{OSR entries}, i.e., a transformation that yields a more optimized version of the code at runtime.
...
The LLVM intermediate representation is a language-independent set of instructions that also provides a type system.
...
\subsection{Benefits of OSR in LLVM}
...
\citean{lameed2013modular} claim to support optimization and reoptimization, as well as deoptimization by going back to the previous version of the function.
...
The McOSR library on-stack replacement implementation fits into the regular JIT infrastructure provided by LLVM as described in Figure \ref{McOSRArchitecture} taken from~\cite{lameed2013modular}. 
\begin{figure}[h]
\centering
...
\subsection{OSR Kit for LLVM}\label{describeOSRKit}
...
The \codekey{destFunGenerator} is the $f_{stub}$ function and is of type \codekey{DestFunGenerator}, described in Figure \ref{fig:destfungenerator}.\\
...
Both of them rely on a \codekey{gen} function to generate the continuation function when the OSR transition is fired.
...
Figures \ref{fig:simplifiedoriginalir} and \ref{fig:simplifiedinstrumentedir} provide an example of OSR instrumentation.
In Figure \ref{fig:simplifiedoriginalir}, we have the R declaration of two functions, \codekey{f} and \codekey{g}, and the simplified LLVM IR generated for \codekey{g} in RJIT.
Figure \ref{fig:simplifiedinstrumentedir} presents the instrumented version of \codekey{g}, in which a resolved OSR point has been inserted.
Lines 5 and 6 correspond to the OSR condition.
In this example, the OSR is fired if line 1 does not yield the same function for \codekey{f} as the one that was used by the compiler when it generated the instrumented function.
If the OSR is fired, the execution jumps to the \codekey{OSR\char`_fire} basic block and calls the continuation function, namely \codekey{OSRCont}.
The OSR point \codekey{OSR\char`_fire} calls the continuation and propagates its return value line 15.
The \codekey{OSR\char`_split} block corresponds to the regular continuation of the function \codekey{f} when the OSR is not fired.\\

\begin{figure}[h]
\includecode{Code/original.llvm}
\caption{Simplified original LLVM IR.}
\label{fig:simplifiedoriginalir}
\end{figure}

\begin{figure}[h]
\includecode{Code/fromInstrumented.llvm}
...
The OSR Kit instrumentation inserts a special basic block, called \codekey{OSR\char`_ENTRY}, at the beginning of the continuation function (see Figure \ref{fig:simplifiedcontinuationir}).
The \codekey{OSR\char`_ENTRY} is responsible for executing the \textit{compensation code} and jumping to the correct block inside the continuation function.
...
The continuation instruction, i.e., the instruction to which the \codekey{OSR\char`_ENTRY} jumps to, is assured by the OSR Kit library to be the first instruction in the destination block.
Everything between the \codekey{OSR\char`_ENTRY} and the continuation block becomes dead code.
...
\begin{figure}[h]
\includecode[asm]{Code/contInstrumented.c}
