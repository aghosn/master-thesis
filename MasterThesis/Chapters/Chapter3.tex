...
Figure \ref{SSA example} provides an example where we have to choose between two possible values for a variable after merging of two control flow branches.\\
...
The implementation has been tested in the McVM/McOSR\cite{chevalier2010optimizing, McVM}, an open source VM and JIT for MATLAB, built on LLVM.
...
\citean{lameed2013modular} claim to support optimization and re-optimization, as well as deoptimization by going back to the previous version of the function.
...
The \textit{destFunGenerator} is the $f_{stub}$ function and is of type \textit{DestFunGenerator}, described in Figure \ref{fig:destfungenerator}.\\
...
In Figure \ref{fig:simplifiedoriginalir}, we have the R declaration of two functions, $f$ and $g$, and the simplified LLVM IR generated for $g$ in RJIT.
Figure \ref{fig:simplifiedinstrumentedir} presents the instrumented version of $g$, in which a resolved OSR point has been inserted.
Lines 5 and 6 correspond to the OSR condition.
In this example, the OSR is fired if line 1 does not yield the same function for $f$ as the one that was used by the compiler when it generated the instrumented function.
...
The OSR Kit instrumentation inserts a special basic block, called \textit{OSR ENTRY}, at the beginning of the continuation function (see Figure \ref{fig:simplifiedcontinuationir}).
