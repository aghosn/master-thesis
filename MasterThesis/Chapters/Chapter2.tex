...
In order to do so, the $f_{base}$ starts by checking a flag to check if it is performing an OSR or serving a regular function call. 
...
%For deoptimization it is a requirement since hard to reverse an opt transformation
    %Even harder if several opt performed

\section{The Deoptimization case}
%yields a lot of questions, where we exit, trade offs in performance?
This section...
...
OSR strives to enable common optimizations performed by static programming language compilers to dynamic language compilers.
...
It enables to eliminate the overhead of a function call and return, and enlarge the scope considered for further optimizations.
...
Some virtual machines, such as Graal EXAMPLES AND REF, CONTINUE SENTENCE.\\
...
CONTINUE. MAKES SENSE SINCE FAILURE OF ASSUMPTION, RESUME FROM BEGINNING TO REGENERATE PROFILING DATA.
...
%Still pretty slow.
Another option is to OSR exit to a based, compiled, and instrumented version of the function.
EXAMPLES WEBKIT etc. 
GOOD INTERMEDIARY SOLUTION; STILL SOME PERFORMANCE IMPROVEMENT OVER THE INTERPRETER; CAN INSTRUMENT THE BASE VERSION TO HAVE OSR ENTRIES; TAKE AWAY POSSIBLE DEPENDENCIES BETWEEN OPTIMIZATIONSM
